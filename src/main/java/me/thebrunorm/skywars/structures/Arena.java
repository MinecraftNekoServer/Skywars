...
public class Arena {
	...
	public boolean joinPlayer(Player player) {
		...
		if (spawn == null) {
			player.sendMessage(Messager.getMessage("SPAWN_NOT_SET", team.getNumber(), this.map.getName()));
			return false;
		}
		...
		for (final SkywarsUser player : this.getAlivePlayers()) {
			...
			Bukkit.getScheduler().runTaskLater(Skywars.get(), () -> {
				...
				if (SkywarsEconomy.getEconomy() != null && playMoney > 0) {
					...
					player.getPlayer().sendMessage(MessageUtils.getMessage("YOU_WON_MONEY_FORMAT", SkywarsUtils.formatDouble(playMoney)));
				}
			}, 20 * 10);
		}
